...
class HelpMenu(ListPageSource):
    ...
    async def write_page(self, menu, fields=[]):
        ...
        embed = Embed(title="Help", description="Krinio Help Menu!", colour=self.ctx.author.colour)
...
class Help(Cog):
    ...
    async def show_help(self, ctx, cmd: Optional[str]):
        """Gives you some help on the Krinio commands!"""
        if cmd is None:
            ...
            if (command := get(self.bot.commands, name=cmd)):
                ...
                await ctx.send("That command does not exist.\nTry looking through `k!help` to see the actual command name, and not the alias.")
