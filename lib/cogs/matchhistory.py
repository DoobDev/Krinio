...
class MatchHistory(Cog):
    ...
    async def crew_won(self, ctx):
        feed = await self.bot.fetch_channel(db.field("SELECT MatchHistory from guilds WHERE GuildID = ?", ctx.guild.id))

        await ctx.channel.purge(limit=1)

        embed = Embed(title="Imposters Lost!", colour=Color.red(), timestamp=datetime.utcnow())
    ...
    async def imposter_won(self, ctx):
        ...
        embed = Embed(title="Imposters Won!", colour=Color.gold(), timestamp=datetime.utcnow())
        embed.set_footer(text=f"Reported By: {ctx.author}", icon_url=ctx.message.author.avatar_url)
