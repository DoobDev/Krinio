...
class Meta(Cog):
    ...
    @command(
        ...
        brief="Get a link to the Krinio support server.",
    )
    ...
    @command(
        ...
        brief="Gives a link to invite Krinio to your server.",
    )
    async def krinio_invite_link(self, ctx):
        """Gives you a link to invite Krinio to another server!"""
        await ctx.send(
            "You can invite the bot here! :link: <https://discord.com/oauth2/authorize?client_id=744377689095536750&scope=bot&permissions=271674430>"
        )
    ...
    async def update_command(self, ctx, *, update: str):
        ...
        if ctx.author.id == owner_id:
            ...
            await ctx.send(
                f"You don't have permissions to give updates about Krinio\nType `{prefix[0][0]}help update` for more info."
            )

    async def show_bot_info(self, ctx, patreon_status):
        embed = Embed(
            title="Krinio Info", colour=ctx.author.colour, timestamp=datetime.utcnow()
        )
